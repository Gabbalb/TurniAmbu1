...
- **Generazione Default 2 Equipaggi**: Aggiornata la logica di inizializzazione automatica dei turni (`ensureShiftsExistForDates` in `api.js`) in modo da generare per default 2 equipaggi per ciascuna fascia oraria (se registrati e attivi nel roster) anziché limitarsi al primo.
